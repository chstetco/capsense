...
class AssistiveEnv(gym.Env):
    def __init__(self, robot=None, human=None, task='', obs_robot_len=0, obs_human_len=0, time_step=0.02, frame_skip=5, render=True, gravity=-9.81, seed=1001):
        self.task = task
    ...
    def setup_cap_camera(self, camera_eye=[0.5, -0.75, 1.5], camera_target=[-0.2, 0, 0.75], fov=60, camera_width=1920//4, camera_height=1080//4):
        self.camera_width = camera_width
        self.camera_height = camera_height
        n = 0.01
        f = 0.5
        ...
        halfY = (fov/2) * np.pi / 180
        t = (0.5/2) * np.tan(halfY)
        b = -t
        r = t
        l = -r

        ortho_matrix = np.array([[2/(r-l), 0, 0, -(r+l)/(r-l)], [0, 2/(t-b), 0, -(t+b)/(t-b)], [0, 0, -2/(f-n), -((f+n)/(f-n))], [0, 0, 0, 1]])
        self.view_matrix = p.computeViewMatrix(camera_eye, camera_target, [0, 1, 0], physicsClientId=self.id)
        projection_matrix = p.computeProjectionMatrixFOV(fov, camera_width / camera_height, 0.01, 1, physicsClientId=self.id)
        projection_matrix = np.reshape(projection_matrix, (4,4))
        self.projection_matrix = np.reshape(np.dot(projection_matrix, ortho_matrix), (16,1))

    def render_depth_img(self, view_matrix, light_pos=[0, -3, 1], shadow=False, ambient=0.8, diffuse=0.3, specular=0.1):
        #assert self.view_matrix is not None, 'You must call env.setup_camera() or env.setup_camera_rpy() before getting a camera image'
        w, h, img, depth, _ = p.getCameraImage(self.camera_width, self.camera_height, view_matrix, self.projection_matrix, lightDirection=light_pos, shadow=shadow, lightAmbientCoeff=ambient, lightDiffuseCoeff=diffuse, lightSpecularCoeff=specular, physicsClientId=self.id)
        img = np.reshape(img, (h, w, 4))
        depth = np.reshape(depth, (h, w))
        return img, depth

    def setup_cap_sensor(self, camera_eye=[0.5, -0.75, 1.5], camera_target=[-0.2, 0, 0.75]):
        camera_width = 64
        camera_height = 64
        #self.camera_width = camera_width
        #self.camera_height = camera_height
        ...
        projection_matrix = p.computeProjectionMatrixFOV(fov, camera_width / camera_height, nearfield, farfield, physicsClientId=self.id)

        assert view_matrix is not None, 'You must call env.setup_camera() or env.setup_camera_rpy() before getting a camera image'
        w, h, img, depth, _ = p.getCameraImage(camera_width, camera_height, view_matrix,
                                               projection_matrix, lightDirection=camera_eye, shadow=False,
                                               lightAmbientCoeff=0.8, lightDiffuseCoeff=0.3,
                                               lightSpecularCoeff=0.1, physicsClientId=self.id)
        img = np.reshape(img, (h, w, 4))
        depth_buffer = np.reshape(depth, (h, w))
        depth = 100 * (farfield * nearfield) / (farfield - (farfield-nearfield) * depth_buffer)
        return img, depth
